...
from s4.src.models.nn import Activation, DropoutNd, Normalization
from s4.src.models.sequence.backbones.block import SequenceResidualBlock
from s4.src.models.sequence.kernels.fftconv import FFTConv
...
class MultiHeadEMA(nn.Module):
    ...
    - The only semantic change is removing the final SiLU activation,
      which is handled by the caller module (e.g. s4.src.models.sequence.mega.MegaBlock).

    It is possible to recreate the functionality of MultiHeadEMA by using other modular blocks,
    in particular the s4.src.models.sequence.kernels.fftconv.FFTConv block
    together with s4.src.models.sequence.kernels.kernel.EMAKernel.
    """
